...
- `VIX_us_stock.csv`：最新日期 ≥ 今天-3 天则命中；过期后增量拉取缺失天数
...
# 溢价率监控告警（配合 GitHub Actions 外部定时触发）
python scripts/fund_alert.py                                 # 需设置 SMTP 等环境变量
python scripts/fund_alert.py --dry-run                       # 仅打印，不推送
```

## GitHub Actions 外部定时触发

通过 cron-job.org 外部定时调用 API 触发 workflow，每天早上 08:30（北京时间，周一至周五）自动执行 `scripts/fund_alert.py`，检查各基金最新溢价率并推送邮件日报（含溢价率趋势图），每条基金附带买入区间建议。
...
2. **配置 cron-job.org**
   ...
   - 新建任务，注意时区设置：
     ...
     | Method | `POST` |
      ...
      cron 表达式 `30 08 * * 1-6` 才是北京时间 08:30。若不改时区，
      需将 cron 改为 `30 0 * * 1-6`（UTC 00:30 = BJT 08:30）。
...
- VIX 数据通过 Yahoo Finance 获取（yfinance），增量缓存，不重复拉全量
