...
| `fund_rating.py` | 基金多维度评级（规模、费率、成立时间、溢价偏离度） |
...
## GitHub Actions 外部定时日报

通过 cron-job.org 外部定时触发，每天早上 08:30（北京时间，周一至周五）自动执行 `fund_alert.py`，检查各基金溢价率 + 纳指100回撤分析，推送邮件日报（含趋势图表）。
...
| `fund_{symbol}_overview.csv` | 7 天内创建过（规模/费率等信息） |
